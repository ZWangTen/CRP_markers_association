...
#write.csv(res,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240629_slp_dis_wo.csv',row.names = F)
```
...
cc <- tab %>% mutate(condition='cog_change')
res <- rbind(diab,hyp,cg,cc)
#write.csv(res,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240629_slp_dis_w.csv',row.names = F)
```
...
#write.csv(wprs,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240629_wprs.csv',row.names = F)
```
...
d <- d %>% remove_rownames() %>% column_to_rownames('CpG') %>% select(-CHR)
d <- as.data.frame(t(d))
...
## Sensitivity analysis - Adjust for cell types 
```{r}
# Read data
pheno<- read.csv('~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Data/20240629_pheno.csv')
pheno$DIABETES2_INDICATOR <-relevel(as.factor(pheno$DIABETES2_INDICATOR), ref = 'Normal' )
...
for (i in 1:14){
  if (i<11){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono'))) #Zmrs_HEnet
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 11:12){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono')))
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
...
#write.csv(mrs,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240701_MRS_cell.csv',row.names = F)
