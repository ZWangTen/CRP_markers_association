...
# Diabetes
tab <- data.frame(pheno=c('SLPA54','SLPA91','SLPA92','Insomnia','LongSlp'),
                  coef=rep(1,5), p.val=rep(1,5),lower=rep(1,5),upper=rep(1,5))

for (i in 1:5){
  form <- as.formula(paste0(tab$pheno[i], paste('~ DIABETES2_INDICATOR + AGE + GENDER + BMI + CENTER + BKGRD1_C7 + EV1 + EV2 + EV3 + EV4 + EV5')))
   if (i<4){
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
  } else if (i==4) {
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
  } else {
    form <- as.formula(paste0('cDur', paste('~ DIABETES2_INDICATOR + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
...
# Hypertension
for (i in 1:5){
    form <- as.formula(paste0(tab$pheno[i], paste('~ HYPERTENSION + AGE + GENDER + BMI + CENTER + BKGRD1_C7 + EV1 + EV2 + EV3 + EV4 + EV5')))
   if (i<4){
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
   } else if (i==4) {
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
  } else {
    form <- as.formula(paste0('cDur', paste('~ HYPERTENSION + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
  }
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
...
for (i in 1:5){
    ...
    form <- as.formula(paste0('cDur', paste('~ global_cog_score + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
  }
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
...
for (i in 1:5){
    ...
    form <- as.formula(paste0('cDur', paste('~ global_cog_score_change + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
  }
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
...
#write.csv(res,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240704_slp_dis_wo.csv',row.names = F)
```
...
for (i in 1:5){
    form <- as.formula(paste0(tab$pheno[i], paste('~ DIABETES2_INDICATOR + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7 + EV1 + EV2 + EV3 + EV4 + EV5'))) #Zmrs_HEnet
    if (i<4){
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    } else if (i==4) {
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
  } else {
    form <- as.formula(paste0('cDur', paste('~ DIABETES2_INDICATOR + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
  }
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
...
# Hypertension
for (i in 1:5){
    form <- as.formula(paste0(tab$pheno[i], paste('~ HYPERTENSION + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7 + EV1 + EV2 + EV3 + EV4 + EV5'))) #Zmrs_HEnet
   if (i<4){
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
   } else if (i==4) {
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
  } else {
    form <- as.formula(paste0('cDur', paste('~ HYPERTENSION + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
  }
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
...
# Cognitive score baseline
for (i in 1:5){
    form <- as.formula(paste0(tab$pheno[i], paste('~ global_cog_score + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7 + EV1 + EV2 + EV3 + EV4 + EV5'))) #Zmrs_HEnet
    if (i<4){
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    } else if (i==4) {
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
  } else {
    form <- as.formula(paste0('cDur', paste('~ global_cog_score + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
  }
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
...
# Cognitive score change
for (i in 1:5){
    form <- as.formula(paste0(tab$pheno[i], paste('~ global_cog_score_change + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7 + EV1 + EV2 + EV3 + EV4 + EV5'))) #Zmrs_HEnet
    if (i<4){
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    } else if (i==4) {
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
  } else {
    form <- as.formula(paste0('cDur', paste('~ global_cog_score_change + Zmrs_corrected + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
  }
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
...
cc <- tab %>% mutate(condition='cog_change')
res <- rbind(diab,hyp,cg,cc)
#write.csv(res,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240704_slp_dis_w.csv',row.names = F)
```
...
```{r}
tab <- data.frame(pheno=c('lgCrp', 'SLPA54', 'SLPA91', 'SLPA92', 'Insomnia',
                          'DIABETES2_INDICATOR', 'HYPERTENSION', 'LongSlp'),
                  coef=rep(1,8), p.val=rep(1,8), lower=rep(1,8), upper=rep(1,8))

for (i in 1:8 ){
  if (i<5){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + wsum_all  + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5'))) #Zmrs_HEnet
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 5:7){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + wsum_all + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i ==8 ) {
    form <- as.formula(paste0('cDur', paste('~ Zmrs_corrected + wsum_all + AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5')))
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
...
#write.csv(wprs,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240704_wprs.csv',row.names = F)
```
...
### Read Data
```{r}
d<-readRDS('~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Data/Lasso data/Enet_corrected.rds')
d <- d %>% remove_rownames() %>% column_to_rownames('CpG') 
d <- d[,-2701]
d <- as.data.frame(t(d))
...
p2$SLPA54<-ifelse(p2$SLPA54>15,'sOsa','mOnorm')
#p2$SLPA54<-log(p2$SLPA54+1)
...
kcvlasso <- cv.glmnet(x, y, nfolds = 5,
                      ...
                      penalty.factor = p.fac) #family='binomial',type.measure = "class",
...
### Data preparation
```{r}
sdata <- read_csv('~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Data/20240629_pheno.csv')
#ev5 <- read.csv("/Volumes/Sofer Lab/HCHS_SOL/Ancestry_files/UW_GAC_DMDA_20180516_local_ancestries/subject_annotation_2017-09-05.csv")
#names(ev5)[2] <- 'SOL_ID'
...
sdata.all <- read.csv('~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Data/20240604_phen_all.csv')
sdata.all<-sdata.all[complete.cases(sdata.all$SLPDUR),]
sdata.all<-sdata.all[complete.cases(sdata.all$BMI),]
```

### Fit model
```{r}
####### MRS CRP
mrs_gam <- mgcv::gam(Zmrs_corrected ~ s(SLPDUR,bs='cr', k=3) +AGE + GENDER + BMI + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5, data=sdata)
summary(mrs_gam)
...
#write.csv(preds_df,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/20240629_mrs_spline.csv',row.names = F)

bld_gam <- mgcv::gam(lgCrp ~ s(SLPDUR,bs='cr', k=3) +AGE + GENDER + BMI + CENTER + BKGRD1_C7, data=sdata)
summary(bld_gam)
preds.all <- predict(bld_gam, newdata = sdata, type = 'terms', se.fit = TRUE)
preds_df <- data.frame(SLPDUR = sdata$SLPDUR, 
                       fit = preds.all$fit[,6], 
                       lwr = preds.all$fit[,6] - 1.96 * preds.all$se.fit[,6], 
                       upr = preds.all$fit[,6] + 1.96 * preds.all$se.fit[,6])

bld_gam.all <- mgcv::gam(lgCrp ~ s(SLPDUR,bs='cr', k=3) +AGE + GENDER + BMI + CENTER + BKGRD1_C7, data=sdata.all)
summary(bld_gam.all)
preds.all <- predict(bld_gam.all, newdata = sdata.all, type = 'terms', se.fit = TRUE)
preds_df.all <- data.frame(SLPDUR = sdata.all$SLPDUR, 
                       fit = preds.all$fit[,6], 
                       lwr = preds.all$fit[,6] - 1.96 * preds.all$se.fit[,6], 
                       upr = preds.all$fit[,6] + 1.96 * preds.all$se.fit[,6])
#write.csv(preds_df.all,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/20240629_blood_spline_all.csv',row.names = F)
```
...
sdata$EDS <- ifelse(sdata$ESS>10, 'EDS', 'Norm')
sdata$Insomnia <- ifelse(sdata$WHIIRS<10, 'Norm', 'Insomnia')
sdata$EDS <-relevel(as.factor(sdata$EDS), ref = 'Norm' )
sdata$Insomnia <-relevel(as.factor(sdata$Insomnia), ref = 'Norm' )
...
crp$s.size[9] <- nrow(dt[complete.cases(dt$WHIIRS),])
crp$s.size[6] <- nrow(dt[complete.cases(dt$global_cog_score),])
crp$s.size[7] <- nrow(dt[complete.cases(dt$global_cog_score_change),])
crp$s.size[8] <- nrow(dt[complete.cases(dt$ESS),])
...
#write.csv(crp,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240704_lgCRP_all.csv',row.names = F)
```
...
res$s.size[c(1:3,13:15,19:21)] <- nrow(dt)
...
res$s.size[22:24] <- nrow(l2[complete.cases(l2$lgCrp),])
s2<- short[complete.cases(short$EV1),]
res$s.size[25:27] <- nrow(s2[complete.cases(s2$lgCrp),])
res$s.size[28:30] <- nrow(dt[complete.cases(dt$SLPA54),])
res$s.size[31:39] <- nrow(dt[complete.cases(dt$SLPA91),])
res$s.size[40:42] <- nrow(dt[complete.cases(dt$SLPDUR),])
res$s.size[16:18] <- nrow(dt[complete.cases(dt$WHIIRS),])
#write.csv(res,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240704_PRS_all.csv',row.names = F)
```




## Sensitivity analysis - Adjust for cell types, waist-hip ratio, and smoking
...
pheno$DIABETES2_INDICATOR <-relevel(as.factor(pheno$DIABETES2_INDICATOR), ref = 'Normal' )
pheno$HYPERTENSION <-relevel(as.factor(pheno$HYPERTENSION), ref = 'Normal' )
pheno$CIGARETTE_USE<-as.character(pheno$CIGARETTE_USE)
...
for (i in 1:14){
  if (i<9){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + AGE +BMI+ GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CIGARETTE_USE '))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 13:14) {
    form <- as.formula(paste0('cDur', paste('~ Zmrs_corrected + AGE +BMI+ GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CIGARETTE_USE ')))
    if (i==13){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
cig <- tab %>% mutate(group='Smoking')


for (i in 1:14){
  if (i<9){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + AGE +BMI+ GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono'))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 13:14) {
    form <- as.formula(paste0('cDur', paste('~ Zmrs_corrected + AGE + GENDER +BMI+ CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono ')))
    if (i==13){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
cell <- tab %>% mutate(group='Cell types')

for (i in 1:14){
  if (i<9){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + AGE + GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5+ WAIST_HIP  '))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 9:12){
    form <- as.formula(paste0(tab$pheno[i], paste('~ Zmrs_corrected + AGE + GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + WAIST_HIP')))
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 13:14) {
    form <- as.formula(paste0('cDur', paste('~ Zmrs_corrected + AGE + GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 +  WAIST_HIP')))
    if (i==13){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
whr <- tab %>% mutate(group='Waist Hip Ratio')
all<-rbind(whr,cell,cig)

#write.csv(all,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240703_MRS_sense.csv',row.names = F)
```
...
for (i in 1:13){
  if (i<8){
    form <- as.formula(paste0(tab$pheno[i], paste('~ lgCrp + AGE +BMI+ GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CIGARETTE_USE '))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 12:13) {
    form <- as.formula(paste0('cDur', paste('~ lgCrp + AGE +BMI+ GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CIGARETTE_USE ')))
    if (i==12){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
for (i in 1:13){
  if (i<8){
    form <- as.formula(paste0(tab$pheno[i], paste('~ lgCrp + AGE +BMI+ GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono'))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 12:13) {
    form <- as.formula(paste0('cDur', paste('~ lgCrp + AGE + GENDER +BMI+ CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono ')))
    if (i==12){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
for (i in 1:13){
  if (i<8){
    form <- as.formula(paste0(tab$pheno[i], paste('~ lgCrp + AGE + GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5+ WAIST_HIP  '))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 8:11){
    form <- as.formula(paste0(tab$pheno[i], paste('~ lgCrp + AGE + GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + WAIST_HIP')))
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 12:13) {
    form <- as.formula(paste0('cDur', paste('~ lgCrp + AGE + GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 +  WAIST_HIP')))
    if (i==12){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
#write.csv(all,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240703_CRP_sense.csv',row.names = F)
```
...
for (i in 1:14){
  if (i<9){
    form <- as.formula(paste0(tab$pheno[i], paste('~ wsum_all + AGE +BMI+ GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CIGARETTE_USE '))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 13:14) {
    form <- as.formula(paste0('cDur', paste('~ wsum_all + AGE +BMI+ GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CIGARETTE_USE ')))
    if (i==13){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
for (i in 1:14){
  if (i<9){
    form <- as.formula(paste0(tab$pheno[i], paste('~ wsum_all + AGE +BMI+ GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono'))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 13:14) {
    form <- as.formula(paste0('cDur', paste('~ wsum_all + AGE + GENDER +BMI+ CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + CD4T + NK + Bcell + Mono ')))
    if (i==13){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
for (i in 1:14){
  if (i<9){
    form <- as.formula(paste0(tab$pheno[i], paste('~ wsum_all + AGE + GENDER  + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5+ WAIST_HIP  '))) 
    mod <- svyglm(form, design = survey_obj, family = "gaussian")
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 9:12){
    form <- as.formula(paste0(tab$pheno[i], paste('~ wsum_all + AGE + GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 + CD8T + WAIST_HIP')))
    mod <- svyglm(form, design = survey_obj, family = quasibinomial(link = "logit"))
    tab$coef[i] <- summary(mod)$coefficients[2,1]
    tab$p.val[i] <- summary(mod)$coefficients[2,4]
    tab$lower[i] <- confint(mod)[2,1]
  ...
  if (i %in% 13:14) {
    form <- as.formula(paste0('cDur', paste('~ wsum_all + AGE + GENDER + CENTER + BKGRD1_C7+ EV1 + EV2 + EV3 + EV4 + EV5 +  WAIST_HIP')))
    if (i==13){
    mod <- svyglm(form, design = survey_long, family = quasibinomial(link = "logit"))
    } else {
      mod <- svyglm(form, design = survey_short, family = quasibinomial(link = "logit"))
...
#write.csv(all,'~/OneDrive - Beth Israel Lahey Health/2023_methCRP/Results/Survey regression/20240703_PRS_sense.csv',row.names = F)
```
